...
export async function POST(request: NextRequest) {
  try {
    /*
     * Verify that someone is signed in.
     */
    ...
    /*
     * Find the member record connected to this login.
     */
     ...
     * the charge must belong to the signed-in member.
     */
    ...
    if (!charge) {
      return NextResponse.json(
        { error: "Charge not found or not available for this account." },
        { status: 404 }
      );
    }
    ...
    const gatewayData =
      (await gatewayResponse.json()) as Record<string, unknown>;
    ...
    const receiptNumber =
      `KBA-${paidAt.slice(0, 10).replaceAll("-", "")}-${reference}`;

    /*
     * Prevent duplicate payment records if the request is repeated.
     */
